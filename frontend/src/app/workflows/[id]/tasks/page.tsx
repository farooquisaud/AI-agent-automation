...
export default function WorkflowTasksPage() {
  ...
  useEffect(() => {
    ...
    setContext({
      ...
      // high-signal task context
      taskStatus: `${tasks.length} task(s) on page ${page} of ${totalPages}`,

      recentActivity: taskSummaries.slice(0, 5).map((t) => ({
        type: "task",
        name: t.name,
        status: t.status,
      })),
  ...
  }, [id, page, totalPages, tasks.length]);
